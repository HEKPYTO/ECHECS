defmodule Echecs.MoveGen do
  ...
  defp generate_quiets_int(%Game{board: board, turn: turn} = game) do
    ...
    |> generate_castling_moves(turn, game)
  end
  ...
  defp generate_castling_moves(acc, turn, game) do
    rights = Map.get(game.castling, turn, [])

    if rights == [] or Game.in_check?(game) do
      acc
    else
      king_sq = if turn == :white, do: 60, else: 4
  ...
  defp legal_int?(game, move_int) do
    ...
    # make_move_on_board_tuple is the fast path
